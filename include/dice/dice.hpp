...
namespace Dc{
    ...
    struct Dice {                      //Literalmente um dado e tudo que um dado pode ter.
        std::string faces;           //possiveis faces.
        char roll();                 //ação de rolar o dado.
    };

    struct Bag {                     // Bolsa que armazena os dados.
    ...
    struct Player {                  //Informações auto-explicativas de cada jogador.
        std::string name;       
        size_t brains_qnt = 0;
        size_t life = 3;
        size_t points = 0;
